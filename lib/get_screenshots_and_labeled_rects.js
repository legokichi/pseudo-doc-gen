// Generated by CoffeeScript 1.11.0
(function() {
  
var fs = require("fs");


// fs APIはコールバックを取るので現代的にPromiseに変換する
// function asynchronous<T, U, V>(fn: (...args: T)=> U, ctx: V): (...args: T)=> Promise<U>
function asynchronous(fn, ctx){
  return function _asyncFn(){
    var args = Array.prototype.slice.call(arguments);
    return new Promise(function(resolve, reject){
      fn.apply(ctx, args.concat(function(err, val){
        if(err){
          reject(err);
        }else{
          resolve(val);
        }
      }));
    });
  };
}

fs.writeFileAsync = asynchronous(fs.writeFile, fs);
;
  var OUTPUT_DIR, i, page;

  OUTPUT_DIR = "output/";

  page = require('webpage').create();

  page.viewportSize = {
    width: 1200 * 2,
    height: 1600 * 2
  };

  i = 0;

  page.onLoadFinished = function() {
    var json, labeled_rects, prefix;
    console.log("======= A =======");
    page.evaluate(function() {
      return Array.prototype.slice.call(document.querySelectorAll('[data-label]')).forEach(function(elm) {
        var $elm, div, height, left, o, ref, top, width;
        $elm = $(elm);
        o = $elm.offset();
        ref = {
          top: o.top,
          left: o.left,
          width: $elm.width(),
          height: $elm.height()
        }, top = ref.top, left = ref.left, width = ref.width, height = ref.height;
        div = document.createElement("div");
        div.style.display = "inline-block";
        div.style.position = "absolute";
        div.style.top = top + "px";
        div.style.left = left + "px";
        div.style.width = width + "px";
        div.style.height = height + "px";
        div.style.border = "1px solid black";
        return document.body.appendChild(div);
      });
    });
    console.log("======= B =======");
    labeled_rects = page.evaluate(function() {
      var elm2$elm;
      elm2$elm = function(fn) {
        return function(elm, i) {
          return fn($(elm), i);
        };
      };
      return {
        text: Array.prototype.slice.call(document.querySelectorAll('[data-label="text"]')).map(elm2$elm(function($elm) {
          var o;
          o = $elm.offset();
          return {
            top: o.top,
            left: o.left,
            width: $elm.width(),
            height: $elm.height()
          };
        })),
        handwriting: Array.prototype.slice.call(document.querySelectorAll('[data-label="handwriting"]')).map(elm2$elm(function($elm) {
          var o;
          o = $elm.offset();
          return {
            top: o.top,
            left: o.left,
            width: $elm.width(),
            height: $elm.height()
          };
        })),
        photo: Array.prototype.slice.call(document.querySelectorAll('[data-label="photo"]')).map(elm2$elm(function($elm) {
          var o;
          o = $elm.offset();
          return {
            top: o.top,
            left: o.left,
            width: $elm.width(),
            height: $elm.height()
          };
        }))
      };
    });
    console.log(json = JSON.stringify(labeled_rects, null, "  "));
    prefix = OUTPUT_DIR + Date.now();
    page.render(prefix + '_screenshot.png');
    fs.write(prefix + "_rect.json", json);
    if (i++ > 100) {
      console.log("end");
      return phantom.exit();
    } else {
      console.log("reload");
      return page.open('http://localhost:8080/');
    }
  };

  page.open('http://localhost:8080/');

  console.log("======= _ =======");

}).call(this);
