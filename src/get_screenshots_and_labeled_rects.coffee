...
page.onLoadFinished = ->
  ...
  labeled_rects = page.evaluate ->
    elm2$elm = (fn)-> (elm, i)-> fn($(elm), i)
    {
      ...
      image:       Array::slice.call(document.querySelectorAll('[data-label="image"]'      )).map elm2$elm ($elm)-> o = $elm.offset(); {top:o.top, left:o.left, width: $elm.width(), height: $elm.height()}
    }
